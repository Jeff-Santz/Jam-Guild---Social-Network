...
void showContactsMenu(User* u, SocialNetwork& sn, NetworkStorage& storage) {
    while (true) {
        system("clear || cls");
        auto contacts = u->getContacts();
        
        cout << "========================================" << endl;
        cout << "       CENTRAL DE CONTATOS (" << contacts->size() << ")" << endl;
        cout << "========================================" << endl;

        if (contacts->empty()) {
            cout << "\n>> Voce ainda nao segue ninguem." << endl;
        ...
        // Tabela de amigos formatada
        cout << left << setw(5) << "ID" << setw(20) << "NOME" << "SUBTITULO" << endl;
        cout << "----------------------------------------" << endl;
        
        for (size_t i = 0; i < contacts->size(); i++) {
            cout << left << setw(5) << i 
                 << setw(20) << (*contacts)[i]->getName() 
                 << (*contacts)[i]->getSubtitle() << endl;
        }
        cout << "----------------------------------------" << endl;
        cout << "-1. Voltar ao Menu Principal" << endl;
        cout << "\nEscolha um amigo para interagir: ";
        ...
        if (choice >= 0 && choice < (int)contacts->size()) {
            Profile* friendPtr = (*contacts)[choice];
            
            // --- SUB-MENU DO AMIGO SELECIONADO ---
            bool inFriendMenu = true;
            while (inFriendMenu) {
                system("clear || cls");
                cout << "========================================" << endl;
                cout << "   ESTOU VENDO: " << friendPtr->getName() << endl;
                cout << "========================================" << endl;
                cout << "Bio: " << friendPtr->getBio() << endl;
                cout << "Desde: " << friendPtr->getStartDate() << endl;
                cout << "========================================" << endl;

                cout << "1. Ver Postagens de " << friendPtr->getName() << endl;
                cout << "2. DESFAZER AMIZADE (Unfollow)" << endl;
                cout << "0. Voltar para a lista" << endl;
                cout << "\nEscolha: ";
                ...
                if (action == 1) {
                    system("clear || cls");
                    cout << "--- POSTS DE " << friendPtr->getName() << " ---" << endl;
                    if (friendPtr->getPosts()->empty()) {
                        cout << "\nNenhuma postagem encontrada." << endl;
                    } else {
                        for (auto* p : *friendPtr->getPosts()) {
                            p->print();
                            cout << "Curtidas: " << p->getLikesCount() << endl;
                            cout << "------------------------------------" << endl;
                        }
                    }
                    cout << "\n[Pressione Enter para voltar]" << endl;
                    ...
                    cout << "\nTem certeza que deseja remover " << friendPtr->getName() << "? (s/n): ";
                    char confirm; cin >> confirm; cleanBuffer();
                    if (confirm == 's' || confirm == 'S') {
                        u->removeContact(friendPtr);
                        cout << ">> Conexao removida. Atualizando banco de dados..." << endl;
                        storage.save(&sn);
                        inFriendMenu = false; // Sai do menu desse amigo pois ele não é mais contato
                    }
                } 
                else if (action == 0) {
                    inFriendMenu = false;
...
int main() {
    ...
    while (option != 0) {
        ...
        // --- CENARIO 2: USUARIO LOGADO (GERENCIANDO O PROPRIO PERFIL) ---
            ...
            // NOTIFICACOES
            if (u && !u->getNotifications().empty()) {
                size_t count = u->getNotifications().size();

                // 2. Lógica de Plural elegante (operador ternário)
                string termo = (count == 1) ? "NOTIFICACAO" : "NOTIFICACOES";

                cout << "\n==========================================" << endl;
                cout << "   [!] VOCE TEM " << count << " " << termo << endl;
                cout << "==========================================" << endl;
                cout << "1. Ler agora" << endl;
                cout << "2. Ir para o menu" << endl;
                cout << "Escolha: ";
                
                int nOp; cin >> nOp; cleanBuffer();

                if (nOp == 1) {
                    cout << "\n--- SUAS " << termo << " ---" << endl;
                    for (auto* n : u->getNotifications()) {
                        cout << "[" << n->getFormattedDate() << "] " << n->getMessage() << endl;
                    }
                ...
                // Limpa a tela para o menu principal não ficar "sujo"
                system("clear || cls"); 
            }
            ...
            cout << "7. Exibir TimeLine" << endl; 
            cout << "8. Exibir Menu de Contatos" << endl; 
            cout << "9. Logout" << endl;
